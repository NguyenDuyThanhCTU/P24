...
function Hotline() {
  ...
  const Hotline = Config?.find((item: any) => item.id === "contact");
  return (
    <>
      {Config && (
        <div className="fixed bottom-7 right-10  box-border flex flex-col gap-5">
          ...
          <div className="flex items-center ">
            <Link
              href={SocialMedia ? SocialMedia?.zalo : "https://zalo.me/"}
          ...
          <Link href={Hotline ? `tel:${Hotline.Hotline}` : "tel:0123456789"}>
            <div className="flex items-center">
              <div className="text-black font-semibold d:flex p:hidden justify-start items-center rounded-full w-[250px]  h-[60px] bg-white shadow-2xl absolute right-5">
                <span className="ml-5">Hotline: {Hotline.Hotline}</span>
              </div>
